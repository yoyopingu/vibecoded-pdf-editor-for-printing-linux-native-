...
- [x] Phase 4.4 — done. Layout: scrollable column of ALL output sheets (`.sheetwrap`), lazily rendered via `_Sheet` (fit-width, `_TV`-painted) + `_SheetColumn` (QScrollArea, no own scrollbar), driven by the shared nav rail through a `_SheetRail` adapter (clone of `_SheetRail`, sheet-index semantics). `_nav_col` stored on the tab (reparentable); `set_single_source()` resolver; `_compute_sheets`/`_arrange_sheets` shared with `_build_plan.run` for preview/run parity (booklet/seq/repeat; 1-up when Anordnung off). Rail reparented in/out in `_switch` with tab-switch/close handling. `PreviewPane` untouched. Auditor: FAIL→fixed (1-up parity when Anordnung off, repeated-page blank-well lazy render, caption range); final PASS (note: `_full` gated on `st_arr.enabled`). Visual audit PASS. Full suite 308/308.
- [x] Phase 4.5 — done. Layout sidebar stages reworked into full-width clickable switch-cards (icon + name/desc + toggle switch-pill, `#stage` QSS with `[on="true"]` accent-soft state) under a "Druckstufen" header; each opts stage panel styled as an `#optCard` (surface-3, border, radius, icon+title+subtitle header); run row in `#runRow` (hairline top, Ausführen flex:1 accent + fixed Stopp). Staging logic untouched. quick 86/86. Visual audit PASS (notes: view-switch truncation, dropdown clipping, button accent shade — cosmetic/deferred).
- [x] Phase 3.2–3.4 — done (see 3.1 entry for all of Phase 3)
- [x] **Phase 5 — done.** Tools morph into sidebar settings panels. 5.1+5.2: `BasePanel.build_controls_widget()` (additive — `build_tool_sidebar`/`build_tool_right_panel`/`apply_split_view_theme` byte-identical); tools mount into sidebar via `SidebarHost.mount("tool", …)`, stack stays viewer(0)/layout(1); sidebar auto-widens to fit tool (224→~414, restored on back). 5.3-5.10: colour_profile + preflight route results to `self.log` (center status/Protokoll, decision 2); other 6 tools needed no change. 5.11: shared tool list in manage/layout — picking a tool keeps the main area on the current view and back restores it. **Grayscale NOT migrated** (owner directive: fragile logic, unique purpose-built preview — kept split-view, falls back to a stack page). quick 87/87. Full Phase 5 committed.
- [x] **Phase 6 — done.** Print dialog: `RadioPill` widget (radiopill.py); top row (wide printer + Bitmap + dpi 300/600/1200/2400 default 600); SEITEN + Skalierung as radiopills; **Farbkonvertierung removed** (decision 1 — combo gone, `"colorconv"` out of prefs.REMEMBERED, spool `-sColorConversionStrategy` stays for the colour_profile tool job); Kopien near top; duplex kept as checkbox+combo (test surface); footer status + summary + buttons right. Known limitation: Bitmap 1200/2400 dpi renders blank (QPrinter pixmap width limit) — offered per concept but print path limited to ~600, renderer fix out of scope. test_printing 47, quick 87.
- [x] **Phase 7 — done.** Merge is the fourth view (decision 3). 7.1: "+"/Ctrl+O open multi-select (1→open, >1→merge). 7.2: FileCard restyled to `.mcard` (`paint_file_card` — first-page thumbnail + filename + page count). 7.3: merge hosts in the main area as a MODE (not a tab) with the pane in `SidebarHost` (`mount("merge", controls_widget)`) + `_FileGridRail` adapter driving the shared nav rail; one pending batch; temp-dir cleanup in `_exit_merge_preview` with `_persist_one()` keeping outputs alive (uniquified names); retired widgets avoid the render-queue heap race. ViewSwitch unchanged. quick 87.
- [x] **Phase 8 — done.** 8.1: icon sweep (glyph buttons → icons.py; `rotated()` helper; titlebar/find/rail/preview/zoom/manage/merge; sidebar tool nav buttons now carry the concept's per-tool icons — drop/layers/num/form/eye/shield/export/zip/puzzle). 8.2: tr() sweep verified (no bare German strings; class constants route via tr() at use site). 8.3: final full suite green; before/after screenshots both themes saved; **grayscale untouched throughout the whole refactor** (owner directive — split-view preview kept). Full suite **309/309**.
